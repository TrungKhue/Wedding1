...
(function(a, b) {
    ...
    const c = a => {
        ...
        "left" == i[0] ? m.x -= q.w + h[0] : "right" == i[0] ? m.x += m.w + h[0] : m.x -= q.w / 2 - m.w / 2, "above" == i[1] ? m.y -= q.h + h[1] : "below" == i[1] ? m.y += m.h + h[1] : m.y -= q.h / 2 - m.h / 2;
        const r = {
                top: 0 > m.y,
                bottom: m.y + q.h > p.h,
                left: 0 > m.x,
        ...
        if (d(() => {
                f.setAttribute("positioned", !0), f.setAttribute("data-placement", i.join(" ")), f.setAttribute("data-pos-overflow", Object.entries(r).reduce((a, [b, c]) => c ? `${a} ${b}` : a, "").trim()), [
            ...
            f.position__trackedScroll = !0, window.addEventListener("scroll", function(b) {
